...
namespace Terminals.Network
{
    ...
    internal class WindowsUserIdentifiers
    {
        internal static string GetCurrentUserSid()
        {
            try
            {
                var fullLogin = Environment.UserDomainName + "\\" + Environment.UserName;
                var account = new NTAccount(fullLogin);
                var sidReference = account.Translate(typeof(SecurityIdentifier));
        ...
        internal static string ResolveUserNameFromSid(string userSid)
        {
            try
            {
                ...
                var userLoginReference = userIdentifier.Translate(typeof(NTAccount));
